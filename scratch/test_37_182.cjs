...
// 37:182 and 1:2
const text_37_182 = "وَالحَمدُ لِلَّهِ رَبِّ العالَمينَ";
const text_1_2 = "الحَمدُ لِلَّهِ رَبِّ العالَمينَ";

console.log("37:182:", text_37_182);
console.log("1:2:", text_1_2);

const res1 = compareTexts(text_37_182, text_1_2, true, true, 'letter');
console.log("\nvowelInsensitive=true, ignoreWhiteSpace=true (37:182 vs 1:2):");
console.log(res1.map(r => r.isDifferent ? `[${r.text}]` : `(${r.text})`).join(''));

const res2 = compareTexts(text_1_2, text_37_182, true, true, 'letter');
console.log("\nvowelInsensitive=true, ignoreWhiteSpace=true (1:2 vs 37:182):");
console.log(res2.map(r => r.isDifferent ? `[${r.text}]` : `(${r.text})`).join(''));

const res3 = compareTexts(text_37_182, text_1_2, false, true, 'letter');
console.log("\nvowelInsensitive=false, ignoreWhiteSpace=true (37:182 vs 1:2):");
console.log(res3.map(r => r.isDifferent ? `[${r.text}]` : `(${r.text})`).join(''));

const res4 = compareTexts(text_1_2, text_37_182, false, true, 'letter');
console.log("\nvowelInsensitive=false, ignoreWhiteSpace=true (1:2 vs 37:182):");
console.log(res4.map(r => r.isDifferent ? `[${r.text}]` : `(${r.text})`).join(''));
